@nhi @nhi-P6-series
Feature: P6705C 嚴重齲齒兒童口腔健康照護複診治療-第一次
    ...
    Scenario Outline: （HIS）90天內，不應有 P6704C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        ...
        Examples:
            ...
            | P6705C       | FM         | DL           | 89                | P6704C           | FM             | 90     | NotPass   |
            | P6705C       | FM         | DL           | 90                | P6704C           | FM             | 90     | NotPass   |
            | P6705C       | FM         | DL           | 91                | P6704C           | FM             | 90     | Pass      |

    Scenario Outline: （IC）90天內，不應有 P6704C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        Given 新增健保醫療:
    ...
    Scenario Outline: （HIS）180 天內，曾經有 P6704C 治療項目
        Given 建立醫師
        Given Wind 24 歲病人
        Given 在過去第 <PastTreatmentDays> 天，建立預約
        Given 在過去第 <PastTreatmentDays> 天，建立掛號
        ...
        Examples:
            ...
            | P6705C       | 11         | MOB          | 179                | P6704C           | 11             | Pass      |
            | P6705C       | 11         | MOB          | 180                | P6704C           | 11             | Pass      |
            | P6705C       | 11         | MOB          | 181                | P6704C           | 11             | NotPass   |

    Scenario Outline: （IC）180 天內，曾經有 P6704C 治療項目
        Given 建立醫師
        Given Wind 24 歲病人
        Given 新增健保醫療:
            | PastDays          | NhiCode          | Teeth          |
    ...
    Scenario Outline: （Disposal）需與91014C或91114C或91020C 併同申報
        ...
        Examples:
            ...
            | P6705C       | 14         | MOB          | 01271C           | NotPass   |
            | P6705C       | 14         | MOB          | 91014C           | Pass      |
            | P6705C       | 14         | MOB          | 91114C           | Pass      |
            | P6705C       | 14         | MOB          | 91020C           | Pass      |
