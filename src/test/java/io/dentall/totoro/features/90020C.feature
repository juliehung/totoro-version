Feature: 90020C 恆牙根管治療（五根以上）
    ...
    Scenario Outline: 全部檢核成功
        ...
        Examples:
            ...
            | 90020C       | 11         | MOB          | 60                | 90015C           | 11             | 60              | 90015C         | 11           | Pass      |
...
#        少了「指定時間內，曾經有指定治療項目 in IC Card」?
    Scenario Outline: 60 天內，曾經有 90015C 治療項目
        ...
        Examples:
            # 待理清 90020C有規定60天內需有90015C項目，請問90015C項目必須同時存在於HIS系統與健保卡內都必需要有記錄，還是擇其一方有記錄就可以算是合於規定內?
            | IssueNhiCode | IssueTeeth | IssueSurface | PastTreatmentDays | TreatmentNhiCode | TreatmentTeeth | PastMedicalDays | MedicalNhiCode | MedicalTeeth | PassOrNot |
            | 90020C       | 11         | MOB          | 60                | 90015C           | 11             | 60              | 90015C         | 11           | Pass      |
            | 90020C       | 11         | MOB          | 61                | 90015C           | 11             | 60              | 90015C         | 11           | Pass      |
            | 90020C       | 11         | MOB          | 60                | 90015C           | 11             | 61              | 90015C         | 11           | Pass      |
            | 90020C       | 11         | MOB          | 61                | 90015C           | 11             | 61              | 90015C         | 11           | NotPass   |
    ...
    Scenario Outline: （HIS）病患牙齒是否有 90020C 健保代碼於60天前已被申報過
        Given 建立醫師
        Given Wind 24 歲病人
        Given 在過去第 60 天，建立預約
        Given 在過去第 60 天，建立掛號
    ...
    Scenario Outline: （IC）病患牙齒是否有 90020C 健保代碼於60天前已被申報過
        Given 建立醫師
        Given Wind 24 歲病人
        Given 在過去第 60 天，建立預約
        Given 在過去第 60 天，建立掛號
    ...
    Scenario Outline: （HIS）病患牙齒是否有 90001C~90003C/90019C~90020C 健保代碼於90天前已被申報過
        ...
        Examples:
            | IssueNhiCode | IssueTeeth | IssueSurface | DayRange | PastTreatmentDays | TreatmentNhiCode | TreatmentTeeth | PassOrNot | InnerPassOrNot |
            # TreatmentNhiCode不用90020C測試，因為90020C同樣的檢查項目但是在60天的條件下測試
            # 恆牙
