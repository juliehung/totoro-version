...
Feature: 92064C 手術去除解剖間隙內異物或牙齒
    ...
    Scenario Outline: （Disposal）每位醫生限申報一次 92064C 代碼
        Given 建立醫師
        Given Scott 24 歲病人
        Given 建立預約
        Given 建立掛號
        ...
        Examples:
            | IssueNhiCode | IssueTeeth | IssueSurface | PassOrNot |
            | 92064C       | 11         | MOB          | NotPass   |

    Scenario Outline: （HIS）每位醫生限申報一次 92064C 代碼
        Given 建立醫師
        Given Scott 24 歲病人
        ...
        And 新增診療代碼:
            ...
            | <PastTreatmentDate> | 3   | <IssueNhiCode> | <IssueTeeth> | <IssueSurface> | 0   | 1.0 | 03  |     |
        Given 建立預約
        Given 建立掛號
        Given 產生診療計畫
        When 執行診療代碼 <IssueNhiCode> 檢查:
