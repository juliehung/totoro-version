...
Feature: 91004C 牙結石清除－全口
    ...
    Scenario Outline: （HIS）90天內，不應有 91005C/91015C~91018C/91089C/91103C~91104C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        Given 在過去第 <PastTreatmentDays> 天，建立預約
        Given 在過去第 <PastTreatmentDays> 天，建立掛號
    ...
    Scenario Outline: （IC）90天內，不應有 91005C/91015C~91018C/91089C/91103C~91104C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        Given 新增健保醫療:
            | PastDays          | NhiCode          | Teeth          |
    ...
    Scenario Outline: （HIS）180天內，不應有 91003C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        Given 在過去第 <PastTreatmentDays> 天，建立預約
        Given 在過去第 <PastTreatmentDays> 天，建立掛號
        ...
        Examples:
            | IssueNhiCode | IssueTeeth | IssueSurface | PastTreatmentDays | TreatmentNhiCode | TreatmentTeeth | GapDay | PassOrNot |
            | 91004C       | UR         | DL           | 179                | 91003C           | FM             | 180     | NotPass   |
            | 91004C       | UR         | DL           | 180                | 91003C           | FM             | 180     | NotPass   |
            | 91004C       | UR         | DL           | 181                | 91003C           | FM             | 180     | Pass      |

    Scenario Outline: （IC）180天內，不應有 91003C 診療項目
        Given 建立醫師
        Given Stan 24 歲病人
        Given 新增健保醫療:
            | PastDays          | NhiCode          | Teeth          |
