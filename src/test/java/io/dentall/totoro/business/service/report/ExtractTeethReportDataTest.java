...
public class ExtractTeethReportDataTest {

    @MockBean
    private ReportDataRepository reportDataRepository;

    @Autowired
    private ExtractTeethReportBuilderService extractTeethReportBuilderService;

    private final AtomicLong disposalIdGenerator = new AtomicLong();

    private final List<String> codes = asList("92013C", "92014C", "92015C", "92016C", "92055C", "92063C");
    ...
    private List<NhiVo> getDataList() {
        // A 有在選定區間內進行拔牙，而且是多次拔牙
        NhiVo voA_1 = newData(LocalDate.of(2021, 1, 18), 1L, 1L, "92013C");
        NhiVo voA_2 = newData(LocalDate.of(2021, 2, 24), 1L, 1L, "92013C");
        // B 有在選定區間內進行拔牙
        NhiVo voB_1 = newData(LocalDate.of(2021, 1, 30), 2L, 2L, "92013C");
        // C 有在選定區間內進行拔牙，但三個月內已有回診
        NhiVo voC_1 = newData(LocalDate.of(2021, 1, 18), 3L, 3L, "92013C");
        NhiVo voC_2 = newData(LocalDate.of(2021, 4, 18), 3L, 3L, "92013C");
    ...
    @Test
    public void testExtractTeethReportData() {
        int gapMonths = 3;
        ...
        ReportDataProvider<ExtractTeethReportSetting, List<ExtractTeethVo>> provider = extractTeethReportBuilderService.getDataProvider();
        FollowupBookSetting bookSetting = new FollowupBookSetting();
        bookSetting.setBeginDate(beginDate);
        bookSetting.setEndDate(endDate);
        ExtractTeethReportSetting setting = new ExtractTeethReportSetting();
        setting.setBookSetting(bookSetting);
        setting.setFollowupGapMonths(gapMonths);
        List<ExtractTeethVo> resultList = provider.get(setting);

        assertThat(resultList.size()).isEqualTo(2);

        List<ExtractTeethVo> patientAList = resultList.stream().filter(r -> r.getPatientId() == 1L).collect(toList());
        assertThat(patientAList.size()).isEqualTo(1);
        ExtractTeethVo patientA = patientAList.get(0);
        ...
        List<ExtractTeethVo> patientBList = resultList.stream().filter(r -> r.getPatientId() == 2L).collect(toList());
        assertThat(patientBList.size()).isEqualTo(1);
        ExtractTeethVo patientB = patientBList.get(0);
    ...
    @Test
    public void testExtractTeethReportDataFilterByDoctor() {
        int gapMonths = 3;
        LocalDate beginDate = LocalDate.of(2021, 1, 1);
        LocalDate endDate = LocalDate.of(2021, 2, 28);
        LocalDate dateAfter3monthsOfEndDate = endDate.plusMonths(gapMonths);
        ...
        List<ExtractTeethVo> patientA = resultList.stream().filter(r -> r.getPatientId() == 1L).collect(toList());
