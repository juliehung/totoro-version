...
public class DailyOwnExpenseSheet extends ExcelSheet {

    private final Counter rowCounter = new Counter();

    public DailyOwnExpenseSheet() {
    }

    @Override
    public void write() {
        Sheet sheet = getReport().getBook().getWorkbook().createSheet("自費");
        ...
        for (OwnExpenseVo vo : data) {
            ...
            cell = row.createCell(cellCounter.get());
            cell.setCellValue(vo.getProcedureMinorType());
            cell.setCellStyle(dataCellStyle);
    ...
    private void createHeader(Sheet sheet, Font font) {
        ...
        sheet.setColumnWidth(5, calculateColumnWith(7, 10));
        sheet.setColumnWidth(6, calculateColumnWith(7, 5));
        sheet.setColumnWidth(7, calculateColumnWith(7, 5));
