...
const NhiDataListRender = ({
  ...
  return Object.entries(validNhiData).map(([key, list], index) => (
    <LazyLoad key={`validNhiData-${index}`} once={true} height={'100%'} overflow={true}>
      <div key={`validNhiData-${index}`} className="render-each-nhi-data-wrap">
        ...
        <div>
          {list.map(({ disposalId, nhiExtendDisposal }, index) => (
            <LazyLoad key={`render-each-nhi-wrap-${index}`} once={true} height={'100%'} overflow={true}>
              <EachWrap
                    ...
                    <CheckboxWrap
                      ...
                      onChange={() =>
                        checkedModalData.find(id => disposalId === id)
                          ...
                          : onCheckedModalDataChange([...checkedModalData, disposalId])
                      }
                    />
                  </div>
                </div>
